...
#include <kameris-formats/cpp/repr_writer.hpp>
...
inline void run(const vector<string> &filenames, const string &output_filename, const string &cgr_type, unsigned k,
	...
	for (size_t i = 0; i < num_seqs; ++i) {
		std::cout<<"sequence..."<<i<<" "<<filenames[i]<<endl;
...
int main(int argc, char *argv[]) {
	cout << "This is generation_cgr (from kameris-backend), " VERSION_LONG "." << endl << endl;
	if (argc != 6 && argc != 7) {
